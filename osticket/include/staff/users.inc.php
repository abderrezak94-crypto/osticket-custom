...
$users->values('id', 'name', 'default_email__address', 'account__id',
    'account__status', 'created', 'updated','cdata__source');
...
<button type="button" class="button action-button" id="sync-ad-btn">
        <i class="icon-refresh"></i> <?php echo __('Synchroniser Active Directory'); ?>
    </button>
    ...
    <thead>
        <tr>
            ...
            <th width="22%"><a  <?php echo $status_sort; ?> href="users.php?<?php
             ...
             <th width="15%"><?php echo __('Source'); ?></th>
...
<td><?php echo Format::htmlchars($U['cdata__source'] ?: 'Invité'); ?></td>
...
$('#sync-ad-btn').click(function () {
    if (confirm("Voulez-vous synchroniser les utilisateurs depuis Active Directory ?")) {
        $.ajax({
            url: '../scripts/sync_ldap_users.php', // Chemin corrigé
            method: 'GET',
            beforeSend: function() {
                $('#sync-ad-btn').html('<i class="icon-spinner icon-spin"></i> <?php echo __("Synchronisation..."); ?>').prop('disabled', true);
            },
            success: function (data) {
            ...
            error: function (xhr) {
                alert("❌ Erreur lors de la synchronisation AD : " + xhr.responseText);
            },
            complete: function() {
                $('#sync-ad-btn').html('<i class="icon-refresh"></i> <?php echo __("Synchroniser Active Directory"); ?>').prop('disabled', false);
            }
        });
    }
});
